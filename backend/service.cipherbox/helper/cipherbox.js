...
const cipherboxConfig = require('../config/cipherbox')
...
module.exports.openCB0Box = (box, key, tsCheck = true) => {
  try {
    ...
    if (box.cbx === exports.cbType.cb0 && Math.abs(getUnixTimestamp() - parseInt(box.ts)) < cipherboxConfig.timestampTolerance) {
...
module.exports.openCB1Box = (box, key, keyType = 'private', tsCheck = true) => {
  try {
    ...
    if (box.cbx === exports.cbType.cb1 && Math.abs(getUnixTimestamp() - parseInt(box.ts)) < cipherboxConfig.timestampTolerance) {
...
module.exports.openCB3Box = (box, key) => {
  try {
    ...
    if (box.cbx === exports.cbType.cb3 && Math.abs(getUnixTimestamp() - parseInt(box.ts)) < cipherboxConfig.timestampTolerance) {
