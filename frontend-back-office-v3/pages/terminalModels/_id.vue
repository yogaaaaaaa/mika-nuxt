  ...
  <div>
    <div class="breadcrumbs-wrapper">
      <v-breadcrumbs>
        ...
        <v-breadcrumbs-item to="/terminalModels" exact>
          Terminal Models
          <span class="ml-3">/</span>
        </v-breadcrumbs-item>
...
import detail from '~/components/terminalModels/detail'
export default {
  components: { detail },
  mixins: [catchError],
  data() {
  ...
  async fetch({ store, params, redirect, $axios }) {
    try {
      let resp = await $axios.$get('/back_office/terminal_models/' + params.id)
