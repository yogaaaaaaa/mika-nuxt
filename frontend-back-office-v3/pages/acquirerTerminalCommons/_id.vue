  ...
  <div>
    <div class="breadcrumbs-wrapper">
      <v-breadcrumbs>
        ...
        <v-breadcrumbs-item to="/acquirerTerminalCommons" exact>
          Acquirer Terminal Commons
          <span class="ml-3">/</span>
        </v-breadcrumbs-item>
...
import detail from '~/components/acquirerTerminalCommons/detail'
export default {
  components: { detail },
  mixins: [catchError],
  data() {
  ...
  async fetch({ store, params, redirect, $axios }) {
    try {
      let resp = await $axios.$get(
        '/back_office/acquirer_terminal_commons/' + params.id
      )
