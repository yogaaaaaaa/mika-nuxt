    ...
    <v-dialog v-model="modalAddForm" width="600">
      <v-card>
        <v-toolbar color="primary" dark flat>
          ...
          <v-spacer/>
          <v-btn icon flat @click="modalAddForm = false">
            <v-icon>close</v-icon>
          </v-btn>
        </v-toolbar>
        <formAdd
...
import formField from '~/components/cardSchemes/formField'
...
export default {
  ...
  data() {
    return {
      resource: 'cardScheme',
      url: '/back_office/card_schemes',
      btnAddText: 'Add Card Scheme',
      headers: [
        {
          text: 'Name',
          align: 'left',
  ...
  methods: {
    ...
    downloadCsv() {
      ...
      this.csvExport(
        `Mika Card Scheme Report ${this.$moment(new Date()).format(
          'YYYY-MM-DD HH:mm:ss'
        )}`,
        this.dataToDownload
      )
